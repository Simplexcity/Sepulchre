// if the player exists, reset persistence
if instance_exists(global.player_start_position) {
	if instance_exists(o_player) {
		...
		o_player.layer = layer_get_id("Instances"); //keep player on correct layer so depth works
	} else {
		//if theres no player, make one
	...
	//reset start position now its been used
